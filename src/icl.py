...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels
    ...
    """
    # YOUR CODE HERE
    supports = [(inp, lab) for inp, lab in zip(support_inputs, support_labels)]
    np.random.shuffle(supports)
    prompt = ""

    if prompt_mode == "qa":
        for inp, lab in supports:
            prompt += f"{inp} In the {lab}."
        return prompt + f"{test_input} In the"

    elif prompt_mode == "none":
        for inp, lab in supports:
            prompt += f"{inp} {lab} "
        return prompt + f"{test_input}"

    elif prompt_mode == "tldr":
        for inp, lab in supports:
            prompt += f"{inp} TL;DR: {lab} "
        return prompt + f"{test_input} TL;DR:"

    elif prompt_mode == "custom":
        for inp, lab in supports:
            prompt += f"{inp} More precisely {lab}."
        return prompt + f"{test_input} More precisely"
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    with torch.inference_mode():
        ...
        new_token = -1
        while new_token not in stop_tokens and len(sampled_tokens) < max_tokens:
            new_token = model(model_input).logits[:, -1, :].argmax()
            if new_token not in stop_tokens:
                model_input = torch.cat(
                    [model_input[0], new_token.unsqueeze(0)], dim=0
                ).unsqueeze(0)
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            text_input = get_icl_prompts(
                                support_x, support_y, test_input, prompt_mode
                            )
                            tokenized_input = torch.tensor(
                                tokenizer.encode(text_input)
                            ).to(DEVICE)
                            tokenized_out = do_sample(
                                model,
                                tokenized_input.unsqueeze(0),
                                stop_tokens,
                                max_tokens,
                            )
                            decoded_prediction = tokenizer.decode(tokenized_out)

                            # END YOUR CODE

                            predictions.append(decoded_prediction.strip())
